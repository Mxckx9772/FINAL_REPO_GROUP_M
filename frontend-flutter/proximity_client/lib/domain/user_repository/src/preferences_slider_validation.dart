...
class PreferencesSliderValidation with ChangeNotifier {
  ...
  void changeSelectStoreCategorie(value, id,
      {bool? check_deselect = false, BuildContext? context}) {
  ...
  void selectAllSubCategories(int categoryIndex) {
    for (var subProductCategory in productCategories![categoryIndex]
        .subCategories) {
  ...
  Future getInitStoreCategories() async {
    ...
    var credentialsBox = Boxes.getCredentials();
    String token = credentialsBox.get('token');
    String? userId = credentialsBox.get('id');
    ...
    try {
      if (userId == null) {
        ...
        dio.options.headers["token"] = "Bearer $token";
        var res =
            await dio.get('$BASE_API_URL/storeCategory/client/$userId');
        if (res.statusCode == 200) {
          print(res.data);
          _storeCategories = [];
          _storeCategories!
