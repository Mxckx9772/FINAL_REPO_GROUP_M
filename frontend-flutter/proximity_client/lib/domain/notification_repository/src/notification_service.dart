...
class NotificationService with ChangeNotifier {
  ...
  Future<void> getNotifications(BuildContext? context) async {
    ...
    try {
      ...
      var res = await dio.get('$BASE_API_URL/notification/$id');
  ...
  Future getOrder(String orderId, BuildContext context,
      {bool? returnOrder = false, bool? refundOrder = false}) async {
    print("ws lanched : $orderId  ");
    _order = null;
    _loadingOrder = true;
    notifyListeners();
    ...
    try {
      Dio dio = Dio();
      dio.options.headers["token"] = "Bearer $token";
      var res = await dio.get('$BASE_API_URL/order/$orderId');
      notifyListeners();
      if (res.statusCode == 200) {
  ...
  Future makeItListSeend() async {
    ...
    try {
      ...
      var res = await dio.post('$BASE_API_URL/notification/update/user/$id',
          data: fd);
      notifyListeners();
      if (res.statusCode == 200) {
        print("notification updated");
