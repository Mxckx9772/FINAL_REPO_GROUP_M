...
const {addOrder,getOrder,client}=require("../setupredis")
//create order

// fonction pour tester sans passer par toute les verifications et la base de données (Rayane)
// changer dans le controlleur 
exports.createOrderTest = async (req)=>{
	try{
	
		if (req.body.delivery==="coursier"){
			await addOrder(req.body.city,req.body.order)
...
exports.createOrder = async (req) => {
	try {
		...
		// si commande par un coursier on j'ajoute à la file d'attente des commandes de la ville.
		if (req.body.deliveryByCoursier){
			await addOrder(req.body.city,req.body.order)
		}
	} catch (error) {
